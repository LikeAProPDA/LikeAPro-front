            ...
            <Form.Group controlId="formBackjoon">
                ...
                <InputGroup className="border rounded">
                    <Form.Control
                        ...
                        value={backjoonId}
                        isInvalid={isDuplicate || (backjoonId.length < 2 && backjoonId.length > 0)}
                        isValid={isDuplicate !== null && !isDuplicate && backjoonId.length !== 0}
                        className="border-0"
                        onChange={(e) => {
                ...
                <Form.Control.Feedback
                    type="invalid"
                    style={{ display: backjoonId.length > 0 && backjoonId.length < 2 ? 'block' : 'none' }}
                >
                    두 글자 이상 적어주세요
                </Form.Control.Feedback>
                <Form.Control.Feedback type="invalid" style={{ display: isDuplicate ? 'block' : 'none' }}>
                    중복된 백준 아이디 입니다.
                </Form.Control.Feedback>
                <Form.Control.Feedback
                    style={{
                        display: isDuplicate !== null && !isDuplicate && backjoonId.length !== 0 ? 'block' : 'none',
                    }}
                >
                    사용해도 괜찮은 백준 아이디 입니다
                </Form.Control.Feedback>
